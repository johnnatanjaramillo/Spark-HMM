...
object CrossValidation {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (new java.io.File(applicationProps.getProperty("root_path") + applicationProps.getProperty("result_models")).exists) {

      sampleClass1 = sparkSession.read.csv(applicationProps.getProperty("sample_class1_folds"))
      ...
      sampleClass0 = sparkSession.read.csv(applicationProps.getProperty("sample_class0_folds"))
      ...
      inInter = scala.io.Source.fromFile(applicationProps.getProperty("root_path") + applicationProps.getProperty("result_models")).getLines.size - 1

    } else {
      /**
        * Make info Class 1
        */
      sampleClass1 = sparkSession.read.csv(applicationProps.getProperty("sample_class1"))
        .where("_c2 == 1").drop("_c2")
      ...
      sampleClass1.write.format("com.databricks.spark.csv").save(applicationProps.getProperty("sample_class1_folds"))

      /**
        * Make info Class 0
        */
      sampleClass0 = sparkSession.read.csv(applicationProps.getProperty("sample_class0"))
        .where("_c2 == 0").drop("_c2")
        .sample(withReplacement = false, applicationProps.getProperty("size_sample").toDouble)
        .withColumnRenamed("_c0", "workitem").withColumnRenamed("_c1", "str_obs")
        .select(col("workitem"), col("str_obs"), row_number().over(Window.orderBy(col("workitem"))).alias("rowId"))
      ...
      sampleClass0.write.format("com.databricks.spark.csv").save(applicationProps.getProperty("sample_class0_folds"))

      hmm.Utils.writeresult(applicationProps.getProperty("root_path") + applicationProps.getProperty("result_models"),
        "N,TP,FP,FN,TN,sensitivity,specificity,accuracy,error,g-mean\n")
      hmm.Utils.writeresult(applicationProps.getProperty("root_path") + applicationProps.getProperty("class1_models"),
        "kfold;M;k;Pi;A;B\n")
      hmm.Utils.writeresult(applicationProps.getProperty("root_path") + applicationProps.getProperty("class0_models"),
        "kfold;M;k;Pi;A;B\n")
    }
    ...
    (inInter until k_folds).foreach(inter => {
      ...
      val trainClass1 = sampleClass1.where("kfold <> " + inter).drop("kfold")
      log.info("Getting data to train Class 0")
      val trainClass0 = sampleClass0.where("kfold <> " + inter).drop("kfold")
      log.info("Getting data to validate Class 1")
      val validClass1 = sampleClass1.where("kfold == " + inter).drop("kfold")
      log.info("Getting data to validate Class 0")
      val validClass0 = sampleClass0.where("kfold == " + inter).drop("kfold")
      log.info("*****************************************************************************************")
      ...
      if (scala.io.Source.fromFile(applicationProps.getProperty("root_path") + applicationProps.getProperty("class1_models")).getLines.size == inter + 2) {
        log.info("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
        log.info("Start Load Model Class 1")
        val stringModel: List[String] = scala.io.Source.fromFile(applicationProps.getProperty("root_path") + applicationProps.getProperty("class1_models")).getLines().toList
        ...
        hmm.Utils.writeresult(applicationProps.getProperty("root_path") + applicationProps.getProperty("class1_iterations"),
          "Fold number: " + inter + "\n")
        hmm.Utils.writeresult(applicationProps.getProperty("root_path") + applicationProps.getProperty("class1_iterations"),
         "iteration;minutes;hours;loglik;antloglik;epsilon\n")

        val tmpModelClass1 = hmm.BaumWelchAlgorithm.run1(trainClass1, value_M, value_k, value_T,
          ...
          inter,
          applicationProps.getProperty("root_path") + applicationProps.getProperty("class1_models_baumwelch"),
          applicationProps.getProperty("root_path") + applicationProps.getProperty("class1_iterations"))
      ...
      if (scala.io.Source.fromFile(applicationProps.getProperty("root_path") + applicationProps.getProperty("class0_models")).getLines.size == inter + 2) {
        log.info("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
        log.info("Start Load Model Class 0")
        val stringModel: List[String] = scala.io.Source.fromFile(applicationProps.getProperty("root_path") + applicationProps.getProperty("class0_models")).getLines().toList
        val arraymodel = stringModel.last.split(";")
        ...
        hmm.Utils.writeresult(applicationProps.getProperty("root_path") + applicationProps.getProperty("class0_iterations"),
          "Fold number: " + inter + "\n")
        hmm.Utils.writeresult(applicationProps.getProperty("root_path") + applicationProps.getProperty("class0_iterations"),
          "iteration;minutes;hours;loglik;antloglik;epsilon\n")

        val tmpModelClass0 = hmm.BaumWelchAlgorithm.run1(trainClass0, value_M, value_k, value_T,
          normalize(DenseVector.rand(value_M), 1.0),
          hmm.Utils.mkstochastic(DenseMatrix.rand(value_M, value_M)),
          hmm.Utils.mkstochastic(DenseMatrix.rand(value_M, value_k)),
          number_partitions, value_epsilon, max_num_iterations,
          inter,
          applicationProps.getProperty("root_path") + applicationProps.getProperty("class0_models_baumwelch"),
          applicationProps.getProperty("root_path") + applicationProps.getProperty("class0_iterations"))
      ...
      resultClass1.write.format("com.databricks.spark.csv").
        save(applicationProps.getProperty("root_path") + applicationProps.getProperty("sample_class1_kfold") + inter)
      ...
      resultClass0.write.format("com.databricks.spark.csv").
        save(applicationProps.getProperty("root_path") + applicationProps.getProperty("sample_class0_kfold") + inter)
